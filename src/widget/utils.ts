...
export function renderMarkdown(text: string): React.ReactNode {
  ...
  try {
    ...
    // 토큰화
    const tokens: Array<{ type: 'bold' | 'italic' | 'code' | 'text' | 'header'; content: string; level?: number }> = [];
    let i = 0;
    ...
    while (i < line.length) {
      ...
      if (i === 0 && line[i] === '#') {
        ...
        // # 다음에 공백이 있어야 헤더로 인식
        if (headerLevel > 0 && headerLevel <= 6 && (i >= line.length || line[i] === ' ')) {
    ...
    tokens.forEach((token) => {
      switch (token.type) {
        case 'bold':
          parts.push(React.createElement('strong', { key: key++ }, token.content));
          break;
        case 'italic':
          parts.push(React.createElement('em', { key: key++ }, token.content));
          break;
        case 'code':
          parts.push(
            React.createElement(
          ...
          break;
        case 'text':
          parts.push(React.createElement('span', { key: key++ }, token.content));
          break;
        case 'header': {
